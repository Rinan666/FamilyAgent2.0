...
- 稳定性发布要求：`chat_sessions` 新存储结构在预发/生产以 `scripts/migrate-chat-session-storage.sql` 为唯一部署真源；启动时 schema auto-init 仅允许 dev 环境开启。
...
- RabbitMQ 宸查厤缃絾褰撳墠涓婚摼璺湭瀹屽叏鍚敤銆?- MinIO 宸查厤缃紝鏈潵鐢ㄤ簬璇煶銆佺収鐗囥€佷綔鍝佸拰瀹跺涵绱犳潗銆?- schema 褰撳墠浠嶆湁 `scripts/init-db.sql` 鍜?Java initializer 涓ゅ眰鏉ユ簮锛岄暱鏈熷簲杩佺Щ鍒扮増鏈寲杩佺Щ銆?- 鍓嶇娴嬭瘯瑕嗙洊浠嶄笉瓒炽€?- 鍏綉 Beta 鍓嶉渶瑕佹垚鏈帶鍒躲€佷娇鐢ㄦ棩蹇椼€侀殣绉佹彁绀哄拰鎺堟潈杈圭晫銆?- 鎴愰暱瀹堟姢涓嶈兘杈撳嚭鍖荤枟璇婃柇锛屽彧鑳借緭鍑烘彁閱掋€佽褰曘€佺鏅拰灏卞尰寤鸿銆?
