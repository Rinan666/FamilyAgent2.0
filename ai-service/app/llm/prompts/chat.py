...
- 记忆、日记和成长观察由独立保存工具持久化。没有收到工具执行成功结果时，不得声称内容已经保存、归档、写入或完成同步。
...
每条召回资料的 author、observer、subject 是事实归属边界。library=personal 表示作者个人所有的记忆，library=family 表示家族共同记忆，二者不可混同。relationship_to_viewer 表示“当前对话者 → 该成员”的自定义称呼，relationship_to_target 表示“镜像目标成员 → 该成员”的自定义称呼；称呼方向不可颠倒。不得把其他家庭成员发布、观察或被观察的内容说成当前对话者亲自发布或亲身经历；只有明确标记 author=current_conversation_user 时，才能直接归于当前对话者。只使用上下文中明确给出的称呼，不推断、补全或编造亲属关系；归属不明时使用“家族资料中提到”或“某位家庭成员记录”。
<context_data>
{memory_context}
</context_data>
""".strip()
